...
Install: `npm i @flexprice/mcp-server-temp`. Or run with one command (no clone or build):

```bash
npx @flexprice/mcp-server-temp start --server-url https://api.cloud.flexprice.io/v1 --api-key-auth YOUR_API_KEY
```
...
```bash
claude mcp add FlexPrice -- npx -y @flexprice/mcp-server-temp start --server-url https://api.cloud.flexprice.io/v1 --api-key-auth YOUR_API_KEY
```
...
```json
"args": ["-y", "@flexprice/mcp-server-temp", "start", "--server-url", "https://api.cloud.flexprice.io/v1", "--api-key-auth", "YOUR_API_KEY", "--mode", "dynamic"]
```
...
```bash
npx @flexprice/mcp-server-temp start --server-url https://api.cloud.flexprice.io/v1 --api-key-auth YOUR_API_KEY --scope read
```
